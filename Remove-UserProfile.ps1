...
function Loading{   #Cosmetic function that outputs a pattern until it finishes deleting files in the background
    [cmdletbinding()]
    $Del = start-job -name DeleteISI -ScriptBlock {remove-item -path "\\networklocation\if\applicable" -force -recurse -ErrorAction stop}
    ...
    Write-Verbose ""\\networklocation\if\applicable deletion state: $($del.state)"
}
...
.EXAMPLE 
    Remove-UserProfile -ComputerName computername -User username
    This will delete the specified user's profile on said machine. 
.EXAMPLE 
    Remove-UserProfile computername -verbose
...
[cmdletbinding()]
param(
 [parameter(valuefrompipelinebypropertyname, mandatory, position = 0)][alias('host','cn')][validatepattern("\d{2,8}-.{1,3}")][string]$ComputerName,   
   ...
   if($ComputerName -eq $env:computername){            #Script restarts at some point; cannot run on localhost
    ...
    if(!($psboundparameters.ContainsValue($user))){            #If user parameter not specified, run below to retrieve currently logged on user on target machine 
        ...
        try{
            ...
            $user = ((Get-CimInstance win32_computersystem -CimSession $session | select -expand username).split('\')[1]) #splits domain and user
    ...
    if($ValidUserBool){            #Execute only if user is valid 
        ...
        try{
            ...
            if(test-connection -ComputerName $computername -ErrorAction stop -quiet){            
                ...
                $DeleteScriptBlock = {            #Deletes HKLM key & user profile file on their machine; not necessary to include \\networklocation\if\applicable here since can be done on our end 
                    ...
                    remove-item -path "C:\Users\$using:user\" -recurse -force -erroraction Stop 
                ...
                Write-Verbose "Attempting to delete HKLM key, C:\Users\$user, and \\networklocation\if\applicable on $($computername.toupper()) for $($user.toupper())."

                loading -verbose            #Cosmetic function called; deletes \\networklocation\if\applicable path with visual on console
